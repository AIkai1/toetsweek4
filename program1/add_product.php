...
if ($_POST) {
    ...
    if (empty($user_id)) {
        $errors[] = 'Gebruikers-ID is vereist.';
    }
    
    if (empty($errors)) {
        $result = $crud->insert_product($naam, $omschrijving, $maat, $afbeelding, $prijs_centen);
        if (is_array($result) && isset($result['error'])) {
            $message = '<div class="bg-red-100 border border-red-400 text-red-700 px-4 py-3 rounded mb-4">' . $result['error'] . '</div>';
        } else {

            $ownershipManager->addOwnership($result, $user_id);
            $message = '<div class="bg-green-100 border border-green-400 text-green-700 px-4 py-3 rounded mb-4">Product succesvol toegevoegd!</div>';
...
<head>
    ...
    <title>Product Toevoegen - Bugimmunity</title>
...
<body class="bg-gray-50">
    ...
    <main class="max-w-2xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div class="bg-white rounded-lg shadow-md p-6">
            <h2 class="text-2xl font-bold text-gray-900 mb-6">Nieuw Product Toevoegen</h2>
            ...
            <form method="POST" class="space-y-6">
                ...
                <div>
                    ...
                    <textarea id="omschrijving" name="omschrijving" rows="4"
                              class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500"><?php echo htmlspecialchars($_POST['omschrijving'] ?? ''); ?></textarea>
                </div>
                ...
                <div>
                    ...
                    <select id="maat" name="maat"
                        ...
                        <option value="xs" <?php echo (($_POST['maat'] ?? '') == 'xs') ? 'selected' : ''; ?>>XS</option>
                        <option value="s" <?php echo (($_POST['maat'] ?? '') == 's') ? 'selected' : ''; ?>>S</option>
                        <option value="m" <?php echo (($_POST['maat'] ?? '') == 'm') ? 'selected' : ''; ?>>M</option>
                        <option value="l" <?php echo (($_POST['maat'] ?? '') == 'l') ? 'selected' : ''; ?>>L</option>
                        <option value="xl" <?php echo (($_POST['maat'] ?? '') == 'xl') ? 'selected' : ''; ?>>XL</option>
                    </select>
                </div>
                ...
                <div>
                    ...
                    <input type="hidden" id="afbeeldingPath" name="afbeelding" value="<?php echo htmlspecialchars($_POST['afbeelding'] ?? ''); ?>">
